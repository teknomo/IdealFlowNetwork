    ...
    * show(): show network
    ...
    def train(self,X,y):
        ...
        for row in range(0,mR):
            ...
            trajSuper=ifn.toMarkovOrder(traj,toOrder=self.markovOrder)
    ...
    def generate(self,category):
        ...
        if str(ifn)!='{}':
            ...
            traj=ifn.orderMarkovLower(trajSuper) # make it first order markov
    ...
    def updateLUT(self,VarVal):
        if VarVal in self.lut.values():
            ...
            hashCode=self.__findKeyInDic__(VarVal,self.lut)
            return hashCode
        else:
            ...
            while True:
                ...
                code1=self.__num_to_excel_col__(int(str(code)[0]))
    ...
    def trajVarVal2Code(self,traj):
        inv_lut=self.__inverseDict__(self.lut)
        lst=[]
        for node in traj:
            if node !="#z#":
                code=inv_lut[node][0]
    ...
    def __predictTrajectory__(self,trajectory):
        ifn=net.IFN()
        trajSuper=ifn.toMarkovOrder(trajectory,toOrder=self.markovOrder)
